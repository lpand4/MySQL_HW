CREATE DATABASE IF NOT EXISTS lesson_4_HW;
...
# ZADANIE 1 Используя JOIN-ы, выполните следующие операции:
...
# ZADANIE 2 Вывести магазин,
# в котором продается кот “Мурзик” (попробуйте выполнить 2 способами)
...
# Zadanie 3 Вывести магазины, в которых НЕ продаются коты “Мурзик” и “Zuza”
SELECT shops.shopname 
FROM shops LEFT JOIN cats
ON shops.id = cats.shops_id
WHERE shops.id
NOT IN (SELECT cats.shops_id FROM cats WHERE name = "Murzik" OR name = "Zuza");
...
SELECT ord_an 
FROM Orders AS O 
WHERE DAY(O.ord_datetime) >= 5 AND DAY(O.ord_datetime) <= 12;

#  ZADANIE 4 Вывести название и цену для всех анализов,
# которые продавались 5 февраля 2020 и всю следующую неделю.
# Если следовать ТЗ, то сделано, если следовать логике, то наверное в таблице заказов должны быть значения ord_id от 1 до 7(кол-во анализов в таблице Analysis) и тогда подвязываться к группе по ан_груп, хотя может их просто не покупали 

SELECT A.an_name AS "Название анализа", A.an_price AS "Стоимость", G.gr_name AS "Группа анализов"
FROM Analysis AS A
RIGHT JOIN Orders AS O
ON A.an_id = O.ord_an
LEFT JOIN groupsan AS G
ON an_group = gr_id
WHERE DAY(O.ord_datetime) >= 5 AND DAY(O.ord_datetime) <= 12;
